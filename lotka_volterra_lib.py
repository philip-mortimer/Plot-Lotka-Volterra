...
def get_new_predators_and_prey(predators, prey, dt, coeffs):
   ...
   """
   half_dt = 0.5 * dt
   midpoint_predators = predators + half_dt * get_delta_predators(predators, 
                                                 prey, coeffs)
   midpoint_prey = prey + half_dt * get_delta_prey(predators, prey, coeffs)
   
   new_predators = predators + dt * get_delta_predators(midpoint_predators,
                                       midpoint_prey, coeffs)
   new_prey = prey + dt * get_delta_prey(midpoint_predators, midpoint_prey,
                             coeffs)
